...
public class PositionGroup extends Master {
	
	public static final Parcelable.Creator<PositionGroup> CREATOR = new Parcelable.Creator<PositionGroup>() {
    	public PositionGroup createFromParcel(Parcel in) {
    		return new PositionGroup(in);
    	}
 
        public PositionGroup[] newArray(int size) {
        	return new PositionGroup[size];
        }
    };
    
    public PositionGroup() {}
    
    public PositionGroup(Parcel in) {
    	super(in);
    	mkbn= MasterConstants.MASTER_MKBN_POSITION_GROUP; //cố định là 4 
    };
    
    @Override
	public void writeToParcel(Parcel dest, int flags) {
    	dest.writeInt(id);
    	dest.writeInt(MasterConstants.MASTER_MKBN_POSITION_GROUP);
    	dest.writeInt(code);
